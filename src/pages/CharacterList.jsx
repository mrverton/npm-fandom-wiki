...
export default function CharacterList() {
  ...
  const filteredArc1Entries = useMemo(() => {
    ...
    return arc1Entries.filter((entry) => {
      if (entry.kind === 'real') {
        ...
        return [c.name, c.shortName, c.role, c.status].filter(Boolean).some((f) =>
  ...
  const showSkeleton = activeTab === 'arc-1' && loading && characters.length === 0
        ...
        <div className="space-y-4">
          ...
          {activeTab === 'arc-2' ? (
              ...
              <div className="flex flex-col items-center justify-center text-center py-16 animate-fade-in">
                ...
                <p className="text-slate-400 text-sm font-medium">Информация ещё не добавлена</p>
                <p className="text-slate-600 text-xs mt-1">Эта арка пока в разработке</p>
              </div>
          ) : showSkeleton ? (
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                {filteredArc1Entries.map((entry, i) =>
                    entry.kind === 'real' ? (
                        <CharacterCard key={entry.key} character={entry.character} index={i} />
                    ) : (
                        <StubCharacterCard key={entry.key} name={entry.name} index={i} />
                    )
                )}
              </div>
          ) : (
